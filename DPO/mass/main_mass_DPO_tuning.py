...
for alpha in ALPHA:
    ...
    for lam in LAM:
        ...
        for i in range(1, N_ITERATIONS + 1):
            data, envj, estj = main_dpo.main(i, alpha, lam)
            tot_env_j += envj
            tot_est_j += estj
            if i == 1:
        ...
        filename_csv = "../csv/mass/DPO/ALPHA={}/LAM={}/stats.csv".format(alpha, lam)
...
filename = "../tuning/mass/DPO/alpha_lam_j.csv"
os.makedirs(os.path.dirname(filename), exist_ok=True)
data_file = open(filename, mode='w')
file_writer = csv.writer(data_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
file_writer.writerow(['alpha', 'lambda', 'j', 'estj'])
for alpha in ALPHA:
    for lam in LAM:
